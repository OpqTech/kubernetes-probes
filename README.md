...
* `opqtech/probes:http-api-v1` - FastAPI application with HTTP probe endpoints
* `opqtech/probes:exec-worker-v1` - Alpine container with Redis client
...
docker build -t opqtech/probes:exec-worker-v1 ./docker/exec-worker

# Push to Docker Hub (optional)
docker push opqtech/probes:exec-worker-v1
```
